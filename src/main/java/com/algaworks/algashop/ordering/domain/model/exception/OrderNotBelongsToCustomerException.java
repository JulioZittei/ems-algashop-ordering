package com.algaworks.algashop.ordering.domain.model.exception;

import com.algaworks.algashop.ordering.domain.model.valueobject.id.CustomerId;
import com.algaworks.algashop.ordering.domain.model.valueobject.id.OrderId;

import static com.algaworks.algashop.ordering.domain.model.exception.ErrorMessages.ERROR_ORDER_NOT_BELONGS_TO_CUSTOMER;

public class OrderNotBelongsToCustomerException extends DomainException {

    public OrderNotBelongsToCustomerException(OrderId orderId, CustomerId customerId) {
        super(String.format(ERROR_ORDER_NOT_BELONGS_TO_CUSTOMER, orderId, customerId));
    }

    public OrderNotBelongsToCustomerException(String message, Throwable cause) {
        super(message, cause);
    }
}
